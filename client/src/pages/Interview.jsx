      ...
      <div className="mx-auto w-full max-w-4xl">
        ...
        {phase === "select" ? (
          <div className="ticket-card mt-6 p-6 sm:p-8 space-y-6">
            ...
            <div className="rounded-xl border border-white/10 bg-surface/50 p-4 sm:p-5 space-y-3">
              <div className="font-heading text-xs uppercase tracking-wider text-text-primary font-bold">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <label
                  className={`flex items-start gap-3 rounded-lg border p-3.5 transition select-none ${
                    !isSpeechSupported
                      ? "opacity-50 cursor-not-allowed bg-ink/5 border-white/10"
                      : voiceEnabled
                      ? "border-gold bg-surface shadow-xs cursor-pointer"
                      : "border-white/10 bg-surface/60 hover:bg-surface cursor-pointer"
                  }`}
                >
                  <input
                    type="checkbox"
                    ...
                    className="mt-0.5 h-4 w-4 rounded border-white/10 text-text-primary focus:ring-stamp-navy"
                  />
                  <div>
                    <span className="font-heading text-xs font-bold text-text-primary flex items-center gap-1.5">
                      <span>🎙️</span> Answer by voice
                    </span>
                    <p className="mt-0.5 text-[11px] text-text-secondary/70 leading-relaxed">
                      {isSpeechSupported
                ...
                <label
                  className={`flex items-start gap-3 rounded-lg border p-3.5 transition select-none cursor-pointer ${
                    cameraEnabled
                      ? "border-gold bg-surface shadow-xs"
            ...
            <div>
              <input
                ...
                className="w-full rounded-lg border border-white/10 bg-surface px-4 py-2.5 text-sm text-text-secondary placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-stamp-navy/30 focus:border-gold"
              />
            </div>
            ...
            {/* Role grid */}
            {rolesLoading ? (
              <p className="mt-6 text-center text-sm text-text-secondary/50">Loading roles…</p>
            ) : filteredRoles.length === 0 ? (
              <p className="mt-6 text-center text-sm text-text-secondary/50">No roles found.</p>
            ) : (
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {filteredRoles.map((r) => (
                  <button
                    ...
                    disabled={busy}
                    className="ticket-card px-4 py-5 text-left transition hover:border-white/10 disabled:opacity-60"
                  >
